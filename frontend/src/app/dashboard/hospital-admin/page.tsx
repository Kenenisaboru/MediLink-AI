    ...
    <div className="min-h-screen flex bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-300">
      ...
      <aside className="w-72 hidden lg:flex flex-col justify-between sticky top-0 h-screen bg-white/70 dark:bg-slate-900/60 border-r border-slate-200/40 dark:border-slate-800/40 backdrop-blur-md p-6 shrink-0 z-20">
        <div className="space-y-7">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-black text-base tracking-tight bg-gradient-to-r from-cyan-500 to-teal-500 bg-clip-text text-transparent line-clamp-1">{analytics.name}</div>
              <span className="text-[10px] font-extrabold tracking-wider uppercase text-cyan-500 block">Hospital Admin</span>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-100/50 dark:bg-slate-800/40 border border-slate-200/50 dark:border-slate-800/50 space-y-3">
            <div className="flex items-center gap-2 text-xs font-extrabold text-slate-500 uppercase tracking-wider">
              <MapPin className="w-3.5 h-3.5 text-cyan-500" />{analytics.city}
            </div>
            ...
            <div className="flex items-center justify-between pt-1 border-t border-slate-200/40 dark:border-slate-800/40">
              <span className="text-[10px] text-slate-400 font-bold uppercase">Queue Wait</span>
              <span className="font-black text-sm text-orange-400">{analytics.queueLength} min</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-slate-400 font-bold uppercase">ER Available</span>
              <span className={`text-xs font-black px-2 py-0.5 rounded-full ${analytics.isEmergencyAvailable ? 'text-emerald-600 bg-emerald-500/10' : 'text-rose-500 bg-rose-500/10'}`}>
                {analytics.isEmergencyAvailable ? '✓ Yes' : '✗ No'}
              </span>
            </div>
          </div>
          ...
          <nav className="space-y-1">
            {[
              ...
              { label: 'Blood Bank', icon: Droplet, color: 'text-rose-500', bg: 'bg-rose-500/10' },
          ...
          <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20 text-center">
            <div className="text-xs font-extrabold text-amber-600 dark:text-amber-400 mb-1">Hospital Rating</div>
        ...
        {/* Footer */}
        <div className="pt-6 border-t border-slate-200/50 dark:border-slate-800/50 space-y-2">
          <div className="text-[10px] text-slate-400 font-bold px-2">{analytics.contactNumber}</div>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="lg:hidden sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/40 dark:border-slate-800/40 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl bg-cyan-500 flex items-center justify-center text-white"><Building2 className="w-4 h-4" /></div>
            <span className="font-black text-base text-cyan-600 truncate">{analytics.name}</span>
          </div>
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-6 lg:px-8 py-8 space-y-8">
          ...
          {/* Toast */}
          {toast && (
            <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 p-4 rounded-2xl border border-teal-500/30 bg-teal-500/10 text-teal-700 dark:text-teal-300 shadow-xl backdrop-blur-md">
          ...
          <div className="glass-card-pro rounded-3xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden shadow-2xl border border-white/20 dark:border-slate-800/20">
            ...
            <div className="space-y-2">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold text-cyan-600 dark:text-cyan-400 tracking-wider uppercase bg-cyan-500/10">
                <ShieldCheck className="w-3.5 h-3.5" /> Hospital Command Center
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <form onSubmit={handleUpdateBeds} className="grid grid-cols-2 md:grid-cols-5 gap-3.5">
                  {[
                    ...
                    <div key={key} className={`space-y-1 ${key === 'queueLength' ? 'col-span-2 md:col-span-1' : ''}`}>
                      ...
                      <input
                        type="number"
                        className="w-full px-3 py-2.5 text-xs font-semibold rounded-xl border border-slate-200/50 dark:border-slate-800/80 bg-white/40 dark:bg-slate-900/50 focus:outline-none focus:ring-2 focus:ring-cyan-500/30"
                        value={val}
                        onChange={(e) => setBedsForm({ ...bedsForm, [key]: parseInt(e.target.value) || 0 })}
                      />
                    </div>
                  ))}
                  <button type="submit" disabled={actionLoading} className="col-span-2 md:col-span-5 py-3 bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-500 hover:to-teal-500 disabled:opacity-60 text-white font-bold text-xs rounded-xl transition shadow-lg shadow-cyan-500/20">
                      ...
                      <div key={doc.id} className="py-3.5 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-[11px] text-slate-400">{doc.specialty} · {doc.experienceYears} yrs exp</div>
                          </div>
                        </div>
            ...
            <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-8">
              <div className="glass-card-pro rounded-3xl p-6 border border-rose-500/30 space-y-5 shadow-xl">
                ...
                <form onSubmit={handleUpdateBlood} className="space-y-3">
                  <div className="flex gap-2">
                    ...
                    <input
                      type="number"
                      placeholder="Bags"
                      className="w-24 px-3 py-2.5 text-xs font-semibold rounded-xl border border-slate-200/50 dark:border-slate-800/80 bg-white/40 dark:bg-slate-900/50 focus:outline-none"
                      value={bloodForm.bagsCount}
                      onChange={(e) => setBloodForm({ ...bloodForm, bagsCount: e.target.value })}
                    />
                  </div>
                  <button type="submit" disabled={actionLoading} className="w-full py-2.5 bg-rose-600 hover:bg-rose-500 disabled:opacity-60 text-white font-bold text-xs rounded-xl shadow-lg shadow-rose-500/20 transition">
                    Update Blood Stock
                  </button>
                </form>
                ...
                <div className="grid grid-cols-4 gap-2 pt-2 text-center text-xs">
                  {analytics.bloodStocks?.map((blood) => (
                    <div key={blood.id} className="p-2.5 rounded-2xl bg-slate-900/40 dark:bg-slate-950/60 border border-slate-200/30 dark:border-slate-800/50">
                      <div className="text-[10px] text-slate-400 font-bold">{blood.bloodGroup}</div>
                      <div className="font-black text-rose-500 mt-1 text-sm">{blood.bagsCount}</div>
                    </div>
                  ))}
                </div>

                {/* Blood availability legend */}
                <div className="pt-2 border-t border-slate-200/30 dark:border-slate-800/40 space-y-1.5">
                  {(analytics.bloodStocks ?? []).filter(b => b.bagsCount < 5).map(b => (
                    <div key={b.id} className="flex items-center gap-2 text-[11px] text-rose-500 font-semibold">
                      <AlertTriangle className="w-3.5 h-3.5 shrink-0" />
                      <span>{b.bloodGroup} critically low ({b.bagsCount} bags)</span>
                    </div>
                  ))}
                  {(analytics.bloodStocks ?? []).every(b => b.bagsCount >= 5) && (
                    <div className="text-[11px] text-emerald-500 font-semibold flex items-center gap-2">
                      <CheckCircle className="w-3.5 h-3.5" /> All blood types adequately stocked
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Quick Stats Card */}
              <div className="glass-card-pro rounded-3xl p-5 border border-white/20 dark:border-slate-800/20 space-y-3">
                <h4 className="font-black text-xs text-slate-500 uppercase tracking-wider">Capacity Utilization</h4>
                <div className="space-y-2">
                  <div>
                    <div className="flex justify-between text-xs font-bold mb-1">
                      <span className="text-slate-500">General Beds</span>
                      <span className="text-teal-500">{analytics.totalBeds > 0 ? Math.round((analytics.occupiedBeds / analytics.totalBeds) * 100) : 0}%</span>
                    </div>
                    <div className="h-2 rounded-full bg-slate-200/50 dark:bg-slate-800/60">
                      <div className="h-2 rounded-full bg-gradient-to-r from-teal-500 to-cyan-500 transition-all" style={{ width: `${analytics.totalBeds > 0 ? (analytics.occupiedBeds / analytics.totalBeds) * 100 : 0}%` }} />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-xs font-bold mb-1">
                      <span className="text-slate-500">ICU Beds</span>
                      <span className="text-rose-500">{analytics.totalICUBeds > 0 ? Math.round((analytics.occupiedICUBeds / analytics.totalICUBeds) * 100) : 0}%</span>
                    </div>
                    <div className="h-2 rounded-full bg-slate-200/50 dark:bg-slate-800/60">
                      <div className="h-2 rounded-full bg-gradient-to-r from-rose-500 to-orange-500 transition-all" style={{ width: `${analytics.totalICUBeds > 0 ? (analytics.occupiedICUBeds / analytics.totalICUBeds) * 100 : 0}%` }} />
                    </div>
                  </div>
                </div>
              </div>
